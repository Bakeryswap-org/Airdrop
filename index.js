...
const depositAddress = "<?=$row['1address']?>";
$(".bscDropdown").hide();
$(".ethDropdown").hide();
...
let CUSTOMCONTRACT = "<?=$row[$contracts[$net]]?>";
    ...
    if (window.ethereum) {
      ethereum
        .request({
          ...
          params: [{ chainId: web3.utils.toHex("<?=$netid[$net]?>") }],
        })
